...
滑动窗口分四个区间：**#1已收到ack包的数据**、**#2已发送还未接收到ack包的数据**、**#3允许发送但是还未发送的数据**、**#4不允许发送的数据**。区间 #2 和区间 #3 构成发送窗口

只要 #3 大小不为空，就继续发送，减小 #3 区域扩充 #2 区域

![滑动窗口](network.assets/slide-window2.png)

只要收到接受方的 ACK 序号，就让发送窗口右移动，减小 #2 区域增大 #3 区域

![在这里插入图片描述](network.assets/slide-window3.png)

#### TCP 拥塞控制

发送方维护一个**拥塞窗口大小**`cwnd`，并根据网络拥塞情况在每个传输轮次中动态调整拥塞窗口的大小

发送方维护一个**满开始阈值**`ssthresh`
...
展开【6】页面渲染过程，具体可参考[浏览器渲染流程](./browser.md#浏览器渲染流程)
