...
可以使用 `auto` 类型说明符定义并初始化变量来让编译器根据初始值自动推断变量的类型，**`auto` 定义会忽略掉顶层 `const` 和引用说明，如果需要定义常量或引用需重新加上 `const/&`**

如果只想推断类型而不想初始化，也可以使用 `decltype(exp)/decltype((exp))` 类型说明符获取表达式的结果类型。双括号的版本会推断出引用类型；而**单括号版本只会在表达式是引用时才会推断出引用**
...
>
> `decltype` 配合 `typedef` 可以方便的为表达式的类型定义类型别名
...
一个最基础的函数定义由返回类型、函数名、形参列表和函数体构成，形如 `T fun_name(T1 t1, T2 t2) {}`。函数的声明则是在函数定义的基础上省略函数体并以分号结尾（函数声明的形参列表可以省略形参名）
...
定义函数时在返回类型前加上 `inline` 关键字将使函数成为==内联函数==，内联函数的调用表达式将在编译阶段被展开为函数体，不会在运行时发生函数调用，适合简单函数

定义函数时在返回类型前加上 `constexpr` 关键字将使函数成为==constexpr 函数==，**`constexpr` 函数隐式是内联的**。`constexpr` 函数的形参和返回类型必须为字面值类型，并且函数体有且仅有一条 `return` 语句，在编译阶段 `constexpr` 函数的调用结果将被推断出来并替换函数调用

> `inline/constexpr` 关键字必须加在函数定义上，函数声明则无强制要求。内联函数和 `constexpr` 函数应该定义在头文件中
