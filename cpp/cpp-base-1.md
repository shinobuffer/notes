...
在类型说明符后面接若干个变量名（逗号分隔）来定义变量；定义变量的时候可同时执行初始化操作，没有初始化操作的变量将执行默认初始化

变量初始化的方式

- ==拷贝初始化==：使用等号的初始化，等号右边的值会作为实参去匹配相关单参数非 `explicit` 构造函数来构造对象，并拷贝到左侧变量中
- ==直接初始化==：直接显式调用构造函数完成值的初始化
- ==值初始化==：通过普通括号或大括号明确了初始化，但存在初始值的缺省，这些缺省初始值的变量将执行值初始化
  - 基础类型的值初始化，其值取决于类型本身，通常为 0
  - 类类型的值初始化，会调用其默认构造函数
- ==默认初始化==（局部、非静态变量）：只指明了类型，没有初始化行为时的兜底行为
  - 基础类型的默认初始化，会直接沿用被分配内存中的值，其值是未定义的
  - 类类型的默认初始化，会调用其默认构造函数

> 只要使用了括号但没有给出具体初始值，就是值初始化，可以简单理解为括号告诉编译器你希望该对象初始化。没有使用括号，就是默认初始化。可以简单理解为放任不管，允许编译器使用默认行为
>
> 对于类类型来说，默认初始化和值初始化没有区别，都是调用其默认构造函数
...
- `vector<T>(int count, [T val])`，构造一个有 `count` 个元素，且每个元素都是 `val` 的向量（如果不提供 `val`，则元素根据其类型进行值初始化）
