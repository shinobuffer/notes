...
> 整型/字符可以通过在类型说明符前添加 `unsigned` 来声明无符号整型/字符
>
...
在类型说明符后面接若干个变量名（逗号分隔）来声明变量；声明变量的时候可同时赋予初始值，没有赋初值的变量将执行默认初始化（具体取决于类型本身）

```c++
int num, *pnum; // 声明整型变量 num 和整型指针变量 pnum
int v1, v2 = 2; // 声明整型变量 v1、v2，其中 v2 初始化为 2，v1 默认初始化为 0
```

以下几种方式都能在声明变量的时候同时执行初始化

```c++
int v1 = 1;
int v2 = {2};
int v3{3}; // 对于内置类型，大括号初始化的方式会检查类型转换时的信息丢失并报错
int v4(4);
```
...
定义在文件顶层的变量将在全局作用域生效，在分离式编译中若其他文件想要共享这个变量，需要在声明时加上 `extern` 关键字，表示该变量在其他地方被定义
...
- 在变量名前加 `&` 声明符来声明引用（左值），**引用声明时必须绑定到已有的对象上（初始化）**；引用相当于对象的别名，操作引用其实就是操作引用绑定的值
- 在变量名前加 `*` 声明符来声明指针，指针是一种存储对象地址的对象，指向地址对应的对象；通过 `*` 操作符来解引用指针访问对象；通过 `&` 操作符来取得对象的地址

```c++
int num = 1;
int &rnum = num; // 为引用绑定对象
int *pnum = &num; // 让指针指向对象
std::cout << *pnum; // 1，解引用指针来访问指针指向的对象
rnum = 2;
std::cout << num; // 2
```
...
>
> **使用/解引用指针时需确保指针确实指向某个存在的对象，建议声明指针时初始化指针**

### const/constexpr 限定符

`const` 限定符用于指示常量，放置于类型说明符或声明符 `*` 后。**常量必须初始化**
...
>
> 底层 `const` 的标识会成为类型的一部分左右着拷贝控制和函数参数的匹配（**非常量可以赋值给底层 `const`，但底层 `const` 只能赋值给底层 `const`**）

`const` 常量默认只会在本文件可见，如果其他文件需要共享该常量，需要在声明时加上 `extern` 关键字，在其他文件使用时对该变量作出 `extern` 声明
...
可以使用 `using` 声明来引入某个名字或使用 `using` 指示来引入命名空间中的所有名字，之后在使用名字的时候就无需指定完整的路径（为方便书写，之后代码默认已进行相关声明或指示，省略路径）
...
> **类的拷贝初始化和直接初始化是两种不同的初始化方式**
>
> - 拷贝初始化：使用等号拷贝，等号右边的值会作为参数去匹配相关**单参数构造函数**并构造副本
> - 直接初始化：直接调用构造函数
...
和其他语言中的字符串用法类似，`s.size()` 获取大小、`s.empty()` 判断是否为空；`string` 之间可以使用 `+` 进行拼接，使用比较运算符以字典序进行比较
...
可以通过范围 for 语句顺序访问 `string` 中的字符；也可以下标索引的形式随机访问字符（**需确保索引的位置不能越界**）。对于字符的处理，`cctype` 头文件提供了许多便捷函数，如判断是否为字母 `isalpha(c)`、大小写转换`tolower/toupper(c)`等
...
字符串的另一种形式字符数组比较特别，字符数组的长度总是会比字符串的长度多 1，用于专门存储尾部的空字符。要处理字符数组，需要用到 C 中的函数（`cstring` 头文件），比如 `strlen()` 获取字符串长度、`strcmp()` 比较字符串、`strcat()` 拼接字符串、`strcpy()` 拷贝字符串。如果可以的话，尽可能使用 `string` 而不是字符数组来表示字符串
...
- `vector<T>(int count, [T val])`，构造一个有 `count` 个元素，且每个元素都是 `val` 的向量（如果不提供 `val`，则元素根据其类型进行默认初始化）
- `vector<T>(iList<T> il)`，其中 `il` 是初始化列表（以大括号的形式表示），以初始化列表中元素为初始值构造向量
...
>
> - **==大括号初始化==会优先尝试使用列表初始化，在无法执行时才回尝试使用匹配的构造函数进行初始化**

`vector` 和同为容器的 `string` 有着不少一致的用法，比如 `v.size()` 获取大小、`v.empty()` 判断是否为空；通过下标索引获取元素引用；使用比较运算符以字典序进行比较等

### 迭代器

不是所有容器都支持下标索引，但所有容器都支持通过迭代器访问内部元素，容器可以通过其 `begin/cbegin()` 或 `end/cend()` 成员函数获取迭代器
...
- 通过自增指向下一个元素，也支持 ±n 的方式获得指向前/后 n 个元素的迭代器
- 迭代器间可以作差来得到两个位置间的距离
...
>
> 如果不需在迭代过程中对元素进行写操作，应该使用常量版本 `cbegin/cend()` 限制写操作

### 数组

如果没有特殊说明，C++ 中的数组特指内置数组，即使用类型说明符和中括号声明的数组。声明数组时需指定数组的维度和维度大小，或者通过初始化让编译器推断维度的大小。**数组的维度和维度大小是类型的一部分**，允许通过范围 for 语句来遍历数组

```c++
// 声明一维字符串数组，长度为 3，第一个元素初始化为 "a"，其余元素默认初始化为空字符串
string strs[3] = {"a"};
// 声明并初始化整型数组（推断为一维数组，长度为 3），元素分别为 1、2、3
int nums[] = {1, 2, 3};
for (auto item : nums) cout << item; // 123
```

> 声明数组时的维度大小必须为 `constexpr` 常量表达式
>
...
通常情况下，**数组可以当作指向数组首元素的指针使用**，允许将数组赋值给对应类型的指针。因为数组是一块连续的空间，指向数组元素的指针可以使用指针的运算操作，支持 ±n 的方式获得指向前/后 n 个元素的指针；指针间也可以通过作差来得到两个位置间的距离
...
>
> 可以使用 `iterator` 头文件提供的 `begin/end(arr)` 方便的获取数组的头元素指针和尾后元素指针，避免手动计算带来的越界风险
>
> `vector` 还有一种构造方式 `vector(pstart, pend)` 可以以数组 `[pstart, pend)` 范围中的值初始化向量（两个参数是指针类型）

**多维数组指的是元素为数组的数组**，而数组等价与数组首元素的指针，因此范围 for 遍历多维数组时，需要将非最内层遍历元素声明为引用类型，避免被转换为指针导致无法遍历

```c++
// 遍历二维数组
int matrix[][] = {{1, 2, 3}, {4, 5, 6}};
for (auto &row : matrix)
  for(auto &item : row) // 这样 item 的类型就会为 (&int)[] 而不是 int[]/*p，可以被遍历
    cout << item;
```
...
大多数运算符都没有规定运算对象的求值顺序，因此需**要避免在赋值操作中使用某个值的同时修改某个值**
...
- 条件中，非布尔值会转换为布尔值
- 初始化过程中，初始值类型会转换为变量的类型；赋值过程中，等号右边的类型会转换为等号左边的类型（非常量类型可转换为常量类型，反过来不行）
- 函数调用时，实参类型会转换为形参的类型，`return` 的类型会转换为返回值类型
...
一个最基础的函数定义由返回类型、形参列表和函数体构成，形如 `T fun_name(T1 t1, T2 t2) {//...}`。函数的声明则是在函数定义的基础上省略函数体并以分号结尾（另外函数声明里也可以省略形参名）

> 函数定义必须配合对应函数声明才能被调用，建议将函数声明放在头文件中
>
> 因为数组/函数不能被拷贝，因此函数的返回类型不能是数组/函数，但可以是指向数组/函数的指针或引用

函数内部的普通局部变量对象（非静态、非动态分配内存）会在退出函数时被自动销毁，如果需要某个变量对象的生命周期贯穿多次函数调用，可以加上 `static` 限定符将其声明为局部静态对象。局部静态对象仅在函数第一次调用时初始化，退出函数后也不会被销毁
...
在声明函数时（注意是声明不是定义），可以为函数指定默认实参（默认实参需位于参数序列的尾部），调用函数时可省略该实参

```c++
void print(vector<string> lines[], char separator = '\n');
```

定义函数时在返回类型前加上 `intern` 关键字将使函数成为内联函数，内联函数的调用表达式将在编译阶段被展开为函数体，不会在运行时发生函数调用，适合规模较小的函数

定义函数时在返回类型前加上 `constexpr` 关键字将使函数成为 `constexpr` 函数，`constexpr` 函数的形参和返回类型必须为字面值类型，函数体有且仅有一条 `return` 语句

> 内联函数和 `constexpr` 函数的关键字必须加在函数定义上，对函数声明无强制要求
>
> 受入参的影响，`constexpr` 函数返回值不一定为常量表达式

### 参数传递

函数的参数传递有两种形式，取决于形参的类型

- ==值传递==非引用类型的形参（包括指针类型），实参将拷贝一份赋值给形参，两者独立
- ==引用传递==引用类型的形参，实参将被绑定到形参上，改变形参的同时也会改变实参

> 对于非基础类型的形参，最好声明成引用来避免大规模拷贝，如果没有修改形参的需要则将形参声明为常量引用

数组不能被拷贝，在作为参数传递时，实际上会作为一个对应的指针拷贝给非引用形参（指针值传递）。也可以把数组形参声明为引用类型，走引用传递的过程
...
- 和 `vector` 一样的是，`initializer_list` 提供了 `size/begin/end()` 成员函数供遍历元素
...
返回值的传递也存在值传递和引用传递两种方式

- ==值传递==以 `return` 的值为初始值，构造返回值类型的副本作为函数调用的结果

- ==引用传递==`return` 引用类型的值，作为函数调用的结果

> 函数不能返回普通局部对象的指针/引用，因为普通局部对象会在退出函数后会被销毁

如果要声明一个返回数组指针的函数，其声明会比较复杂，看上去不是很直观。对于这类返回类型比较复杂的函数，可以将其返回类型后置提升可读性（在原返回类型的位置用 `auto` 说明类型符替代，并且在参数列表后面用 `->` 指示真正的返回类型）
