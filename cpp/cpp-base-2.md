...
- ==成员函数==的声明需要在类内部，定义既可以在类内部也可以在类外部
  - **定义在内部的成员函数本身就是声明，并且是隐式 `inline` 的**
...
一个单参数构造函数暗含着隐式类型转换，即可以将形参类型的值当作类类型值使用，编译器会自动以前者为实参调用构造函数来创建一个类对象完成一次隐式类型转换。同理，多参数构造函数同样存在隐式类型转，该情况下实参序列需要使用大括号包裹

要想阻止构造时的隐式转换，可以将构造函数声明前加上 `explicit` 关键字，**`explicit` 构造函数只能用于直接初始化**

```c++
class ExplicitA {
public:
  explicit ExplicitA(int a, double b);
}

ExplicitA fun() {
  // return {1, 2.0}; // 错误用法，explicit 构造函数禁止隐式转换
  return ExplicitA(1 ,2.0); // 正确用法
}
```
...
Vec::Vec(double x, double y): m_x(x), m_y(y) {}
...
```c++
class Vec {
	// 声明友元函数 print，友元函数可以直接访问私有数据成员 m_x、m_y
...
int Vec::m_static_var = static_fun();
```
...
对类成员取址可以得到一个成员指针，用于给成员指针变量初始化或赋值，如 `double Vec::*px = &Vec::m_x`
...
// 成员指针 px 可以指向 Vec 的浮点类型成员，初始化指向数据成员 m_x（假设 m_x, m_y 现在都是 public 的）
double Vec::*px = &Vec::m_x;
...
智能指针默认初始化为空智能指针，也可以初始化的时候绑定一个指向有效动态内存的内置指针。他们的共有成员函数如下

- `p.get()` ，获取智能指针中托管的内置指针
- `p.swap(p2)/swap(p1, p2)` ，交换两个智能指针

使用 `p.get()` 时需要注意，如果智能指针托管的内存被释放，先前 `p.get()` 得到的内置指针将失效
...
右值是只有值的存在，无法在代码中直接获取到对应的内存地址，多为字面值和表达式求值得到的非引用结果，只要离开了当条表达式就没法追踪
...
考虑到左值可以当右值使用，标准库头文件 [`<utility>`](https://www.apiref.com/cpp-zh/cpp/utility.html) 提供了 `move()` 函数将一个左值转换为一个右值引用。左值通过 `move` 成为右值交给别人，相当于将底层值的生命周期拱手相让，这时候再通过原左值来访问底层值是未定义的，**因此 `move` 以后必须做出不读取原左值的保证**（销毁和写操作是允许的）
