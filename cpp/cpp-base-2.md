...
根据右值的特性，右值引用总是指向一次性将要被销毁的对象。如果构造/赋值的源是一个右值引用，可以直接从右值引用接管底层值，免去副本构造的流程直接实现底层值的移动
...
类定义了自己的移动构造函数，则合成版本的拷贝构造函数将是删除的，反过来也是如此（移动赋值运算符同理），因此如果一个类定义了任一拷贝控制成员，就应该把其他的拷贝控制成员也定义了
